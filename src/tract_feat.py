...
def feat_RASF(pd_tract, number_of_points=15):
    ...
    feat = np.dstack(
        (fiber_array.fiber_array_r, fiber_array.fiber_array_a, fiber_array.fiber_array_s, fiber_array_fs_t))
...
def _region_label_LR(label):
    ...
    commissural_sub_cortical_regions_new = np.arange(CC_regions_new[-1] + 1,
                                                     CC_regions_new[-1] + 1 + len(commissural_sub_cortical_regions))

    # combine left and right
    left_sub_cortical_regions = range(1, 14) + range(17, 21) + range(25,
                                                                     40)  # [2, 4, 5, 7, 8, 10, 11, 12, 13, 17, 18, 26, 28, 30, 31]
    left_sub_cortical_regions_new = np.arange(commissural_sub_cortical_regions_new[-1],
                                              commissural_sub_cortical_regions_new[-1] + 1 + len(
                                                  left_sub_cortical_regions))
    ...
    left_GM_cortical_regions_new = np.arange(left_sub_cortical_regions_new[-1] + 1,
                                             left_sub_cortical_regions_new[-1] + 1 + len(left_GM_cortical_regions))
    ...
    left_WM_cortical_regions_new = np.arange(left_GM_cortical_regions_new[-1] + 1,
                                             left_GM_cortical_regions_new[-1] + 1 + len(left_WM_cortical_regions))
...
def downsample_to_balance(x_data, y_data, num_sample_per_class=None):
    """My understanding: It will reduce the samples
       When num_sample_per_class =None, Random sample from classes, making all classes to have
       the same sample. The balanced number for all classes is the class number with least samples;

       When num_sample_per_class !=None, Random sample from class with more samples, the num_sample_per_class will be
       largest number for all classes; """
...
def upsample_to_balance(x_data, y_data, verbose=False):
    """My understanding: It will increase the samples
       The principle is similar to 'downsample_to_balance' """
    bin_count = np.bincount(y_data)
    ...
    for label in np.unique(y_data):
        ...
        if verbose:
            print("[upsample_to_balance] class", label, "has", len(label_indices), "samples, and will repeat",
                  copy_times, "times.")
...
def upsample_with_copy(x_data, y_data, labels_to_upsample=None, copy_times=None, gaussian_sigma=None, verbose=False):
    ...
    for label, cp_time in zip(labels_to_upsample, copy_times):
        ...
        if verbose:
            print("[upsample_to_balance] class", label, "has", len(label_indices), "samples, and will repeat", cp_time,
                  "times.")
...
def split_data(x_data, y_data, split_rate):
    """split the training and validating data
        e.g., when split_rate = 0.8, train:valid = 4:1"""
    n = x_data.shape[0]

    n_first = int(n * split_rate)

    # np.random.seed(0)
    p_indices = np.random.permutation(n) # shuffle
...
def combine_tract_subdiviations_and_keep_outlier_tracts(y_data, y_names, y_validation=None, verbose=False):
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_Sup_left', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_Sup_right', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_O_Sup_left', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_O_Sup_right', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_CBLM_right', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
    ...
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_O_CBLM_left', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)

    labels_to_combine = [66, 68]  # O-CBLM right
    y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name='T_O_CBLM_right', y_names=y_names,
                                      verbose=verbose)
    if y_validation is not None:
        y_validation, _ = _combine_tracts(y_validation, labels_to_combine)
...
def combine_truepositive_and_falsepositive(y_data, y_names, y_validation=None, verbose=False):
    for y_label, y_name in enumerate(y_names):
        ...
        if 'T_O_' in y_name:
            ...
            y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name=y_names[idx_tp_tract],
                                              y_names=y_names, verbose=verbose)
...
def bilateralize_feature(y_data, y_names, y_validation=None, verbose=False):
    for y_label, y_name in enumerate(y_names):
        ...
        if '_right' in str(y_name):
            ...
            y_data, y_names = _combine_tracts(y_data, labels_to_combine, combined_name=combined_name, y_names=y_names,
                                              verbose=verbose)
...
def compress_labels_and_names(y_train, y_names, y_validation=None):
    """my understanding: it does nothing with training data"""
    y_names_new = list()
    y_train_new = y_train
...
def get_tract_specific_data(tract, y_names, y_data, x_data, y_validation=None, x_validation=None, idx_data=None,
                            idx_validation=None):
    """my understanding: Select data for specific tract. It is not used for now"""
    kept_labels = list()
    for y_label, y_name in enumerate(y_names):
...
def update_y_test_based_on_model_y_names(y_test, y_names, y_names_in_model):
    """My understanding: Used when inputLabel for testing is not None"""
...
def normalize_channel(x_data, pre_max_v_list=None):
    """My understanding: normalize values in different channels"""
    max_v_list = list()
